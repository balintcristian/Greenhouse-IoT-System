...
export default function TemperatureDashboard({ APIEndpoint }) {
  ...
  console.log(`${APIEndpoint}/sensors/temperature/`);
  const fetchSensorIds = async () => {
    try {
      const res = await fetch(`${APIEndpoint}/sensors/temperature/`);
    ...
    try {
      const results = await Promise.all(
        sensorIds.map(async (id) => {
          try {
            const res = await fetch(`${APIEndpoint}/sensors/temperature/${id}`);
            const json = await res.json();
